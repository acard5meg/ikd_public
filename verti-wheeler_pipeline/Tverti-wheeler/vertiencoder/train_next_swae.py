...
from model.swae import SWNTAutoencoder
from model.dataloader import TvertiDatasetAENextToken
...
class Learner:
    ...
    def train(self):
        ...
        # x, next_x, action = next(iter(self.data))
        # macs, params = op_counter(
        #     self.model, sample=(x.to(device=self.device), action.to(device=self.device))
        # )
    ...
    def forward_batch(self, batch):
        ...
        next_patch = batch[1].to(self.device)
        action = batch[2].to(self.device)
        # next_action = batch[3].to(self.device)
        # forward, backward
        recon_patch, z = self.model(patch, action)
        loss = self.criterion(recon_patch, next_patch, z)
        ...
        log_samples = make_grid(
            [
                ...
                for gt, pred in zip(
                    next_patch[:64].cpu(), recon_patch[:64].cpu().detach()
                )
            ],
            nrow=8,
        )
    ...
    def validate(self):
        ...
        for batch in bar:
            ...
            next_action = batch[3].to(self.device)
            # forward, backward
            recon_patch = self.model.generate(patch, action, next_action)
            loss = torch.nn.functional.mse_loss(recon_patch, next_patch)
            self.psnr.update(recon_patch, next_patch)
            log_samples = make_grid(
                [
                    make_grid([gt, pred], nrow=2, value_range=(-1, 1), normalize=True)
                    for gt, pred in zip(next_patch[:64].cpu(), recon_patch[:64].cpu())
                ],
                nrow=8,
            )
    ...
    def init_model(self):
        ...
        model = SWNTAutoencoder(**self.cfg.model)
    ...
    def init_dataloader(self):
        ...
        dataset = TvertiDatasetAENextToken(**self.cfg.dataset)
        self.cfg.dataset.update(self.cfg.val_dataset)
        val_dataset = TvertiDatasetAENextToken(**self.cfg.dataset)
        # creating dataloader
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", default="conf/swae_next", type=str)
