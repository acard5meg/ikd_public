...
def get_conf(name: str):
    ...
    OmegaConf.register_new_resolver(
        "from_yaml",
        lambda address, key, _root_: OmegaConf.load(Path(address).resolve().as_posix()).get(key, _root_),
    )
    OmegaConf.register_new_resolver("eval", eval)
    name = Path(name).resolve().as_posix() if name.split(".")[-1] == "yaml" else name + ".yaml"
...
def read_patch(patch_name: str, robot_height: float = 0.0, height_diff: float = 0.5) -> torch.Tensor:
...
def read_map(map_name: str, robot_height: float = 0.0, height_diff: float = 0.5) -> torch.Tensor:
