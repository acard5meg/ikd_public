...
from model.dt_models import PoseEstimation, BehaviorCloning, IKD
...
class Learner:
    def __init__(self, cfg_dir: str):
        ...
        self.cfg.logger.experiment_name += self.cfg.dataset.task + f"-{'finetuned' if self.cfg.model.finetune else 'frozen'}"
    ...
    def train(self):
        ...
        while self.epoch <= self.cfg.train_params.epochs:
            ...
            log = (((f"{datetime.now():%Y-%m-%d %H:%M:%S} Epoch {self.epoch:03}, "
            ...
            if self.cfg.dataset.task == 'reconstruction':
                log = log[:-1] + f"\t| PSNR: [red]{self.psnr.compute().item():.3f}[/red]\n"
        ...
        # patch, cmd_vel, _ = next(iter(self.data))
        # macs, params = op_counter(
        #     self.model,
        #     sample=(patch.to(device=self.device), cmd_vel.to(device=self.device)),
        # )
    ...
    def forward_batch(self, batch):
        ...
        if self.cfg.dataset.task in ["ikd", 'reconstruction']:
        ...
        # forward, backward
        if self.cfg.dataset.task == "reconstruction":
            z, _ = self.pretext_model(patch, cmd_vel)
        else:
            z, _ = self.pretext_model(patch, cmd_vel)
        if self.cfg.dataset.task == "ikd":
            pred = self.dt_model(z, add_info)
        else:
    ...
    def validate(self):
        ...
        for batch in bar:
            ...
            # forward, backward
            if self.cfg.dataset.task == "reconstruction":
                z, _ = self.pretext_model.generate(patch, cmd_vel, add_info)
            else:
                z, _ = self.pretext_model(patch, cmd_vel)
            if self.cfg.dataset.task == "ikd":
                pred = self.dt_model(z, add_info)
    ...
    def init_model(self, task: str):
        ...
        if task == "fkd":
            self.cfg.model.pose_model.dims.insert(
                0, self.cfg.model.transformer.transformer_layer.d_model
            )
            dt_model = PoseEstimation(self.cfg.model.pose_model)
            # del swae

        elif task == "bc":
            # ic(self.cfg.model.bc_model.dims)
            self.cfg.model.bc_model.dims.insert(
                0, self.cfg.model.transformer.transformer_layer.d_model
            )
            # ic(self.cfg.model.bc_model)
            dt_model = BehaviorCloning(self.cfg.model.bc_model)
            # del swae

        elif task == "reconstruction":
            swae.decoder.train()
            swae.decoder.requires_grad_(True)
            dt_model = swae.decoder
    ...
    def save(self, name=None):
        model = self.dt_model
        if isinstance(self.dt_model, torch.nn.DataParallel):
            model = model.module
