...
from nn import init_weights

# ATC EDITS
# from vertiencoder.model.swae import Encoder, Decoder, SWAutoencoder
from swae import Encoder, Decoder, SWAutoencoder
...
class Tverti(nn.Module):
    def __init__(self, cfg: DictConfig, patch_encoder: nn.Module):
        ...
        # self.patch_encoder = PatchEmbedding(cfg.action_encoder.latent_dim, patch_size=40)
        self.action_encoder = nn.Linear(2, cfg.action_encoder.latent_dim)
        self.pose_encoder = nn.Linear(6, cfg.action_encoder.latent_dim)
        self.action_head = nn.Linear(cfg.action_encoder.latent_dim, cfg.action_encoder.latent_dim)
        self.pose_head = nn.Linear(cfg.action_encoder.latent_dim, cfg.action_encoder.latent_dim)
        ...
        # self.pose_head = nn.Linear(cfg.action_encoder.latent_dim, 6)
    ...
    def forward(
        self, patches: torch.Tensor, actions: torch.Tensor, pose: torch.Tensor = None, mask: torch.Tensor = None
        ...
        # tokens = rearrange([patches, actions, poses], "c b t l -> b (c t) l")
        ...
        if self.training:
            ...
            pred_actions_token = self.action_head(pred_tokens[:, self.block_size + 1: (self.block_size * 2) + 1])
            pred_pose_token = self.pose_head(pred_tokens[:, (self.block_size * 2) + 1:])
            return ctx_pred, pred_patch_token, pred_actions_token, pred_pose_token

        return ctx_pred

    def generate(self, patches: torch.Tensor, actions: torch.Tensor, pose: torch.Tensor,
                 future_actions: torch.Tensor, timesteps: int = 15) -> torch.Tensor:
        # not working
        actions = self.action_encoder(actions)
        poses = self.pose_encoder(pose)
        patches = torch.stack(
        ...
        for i in range(timesteps):
            ...
            b, t, l = tokens.size()
            ctx_token = self.ctx_token.expand(b, -1, -1)
            tokens = torch.cat([tokens, ctx_token], dim=1)

            tokens = self.pos(tokens)
            pred_tokens = self.transformer_encoder(tokens)
            pred_tokens = self.ln(pred_tokens[:, -1, :])
            next_patch = self.patch_head(pred_tokens)
            # pred_pose = self.pose_head(pred_tokens)

            patches = torch.cat([patches, next_patch.unsqueeze(1)], dim=1)[:, 1:, :]
            actions = torch.cat([actions, future_actions[:, i: i+1, :]], dim=1)[:, 1:, :]
            poses = torch.cat([poses, poses], dim=1)
