...
class Deployment_Tverti():
    def __init__(self):
        ...
        self.stats = None
        # Load the statistics
        with open('/home/aniket/Token_prediction/Deployment/Tverti-wheeler/vertiencoder/checkpoint/last_hopes/stats.pkl', 'rb') as f:
    ...
    def model_predict(self):
        ...
        cmd = torch.tensor(self.gt_cmds).mean(dim=0)
        cmd = (cmd - self.stats['cmd_vel_mean']) / self.stats['cmd_vel_std']
        ...
        pose_diff = ((pose_diff.squeeze(0).cpu() * self.stats['pose_diff_std']) + self.stats['pose_diff_mean']) * 0.1
    ...
    def initialize_stack(self):
        ...
        del_pose = (del_pose - self.stats['pose_diff_mean']) / self.stats['pose_diff_std']
        self.prev_robot_pose_m = self.robot_pose
